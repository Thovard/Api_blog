...
blogRouter.get('/posts', async(req, res) => {
    try {
        ...
        let blogs = await Blog.find({}).sort({ createdAt: (sortParam == 'true') ? -1:1 })
...
blogRouter.post('/posts', verifyToken, async(req, res) => {
    try {
...
blogRouter.put("/posts/:id", verifyToken, async(req, res) => {
    try {
        const blog = await Blog.findById(req.params.id)
...
blogRouter.delete("/posts/:id", verifyToken, async(req, res) => {
    try {
        const blog = await Blog.findById(req.params.id)
        if(blog.userId !== req.user.id){
